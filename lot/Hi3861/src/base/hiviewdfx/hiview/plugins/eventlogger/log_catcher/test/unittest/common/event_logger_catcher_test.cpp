...
HWTEST_F(EventloggerCatcherTest, EventloggerCatcherTest004, TestSize.Level3)
{
     ...
     * @tc.steps: step1. create event handler and events
     */
    constexpr int minQuantity = 3200;
    auto ret = StartCreate("EventloggerCatcherTest004", "TEST04_BINDER", "b",
        -1, "", 0, minQuantity);
    if (ret < 0) {
        printf("EventloggerCatcherTest004 is error ret == %d\n", ret);
        FAIL();
    }

    auto fd = JudgmentsFileSize(minQuantity, "EventloggerCatcherTest004");
    if (fd < 0) {
        printf("EventloggerCatcherTest004 JudgmentsFileSize is error ret == %d\n", fd);
        FAIL();
    }
...
HWTEST_F(EventloggerCatcherTest, EventloggerCatcherTest005, TestSize.Level3)
{
    ...
    if (pid == 0) {
        if (prctl(PR_SET_NAME, "EventloggerCatcherTest005") == -1) {
            printf("PR_SET_NAME, EventloggerCatcherTest005_Child is error\n");
        }
        prctl(PR_SET_PDEATHSIG, SIGKILL);
        int volatile temp[memSize] = {0};
        while (true) {
    ...
    auto ret = StartCreate("EventloggerCatcherTest005", "TEST05_APP_STACKTRACE", "s",
        pid, "EventloggerCatcherTest005_Child", 0, minQuantity);
    if (ret < 0) {
        printf("EventloggerCatcherTest005 is error ret == %d\n", ret);
        FAIL();
    }

    auto fd = JudgmentsFileSize(minQuantity, "EventloggerCatcherTest005");
    if (fd < 0) {
        printf("EventloggerCatcherTest005 JudgmentsFileSize is error ret == %d\n", fd);
        FAIL();
    }
