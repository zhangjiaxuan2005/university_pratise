...
/*  // GPIO_6 set LOW at NfcTestEntry(), delay 250us is satisfied
    ...
    IoTGpioSetOutputVal(HI_IO_NAME_GPIO_6, IOT_GPIO_VALUE0);//CSN管脚拉低，延时250us，给芯片上电
    hi_udelay(250);//不要低于这个值
*/
    ...
    // OpenBrother NOT suing this GPIO9
    //hi_gpio_set_ouput_val(9, 1);//CSN管脚拉高
}
...
unsigned int write_fifo_data( unsigned char* data_buff, unsigned char len)
{
    ...
    c081nfc_i2c_write_cmd_addr.sendLen = 2 + len;
    int ret = IoTI2cWrite(id, C081_NFC_ADDR&0xFE, c081nfc_i2c_write_cmd_addr.sendBuf, c081nfc_i2c_write_cmd_addr.sendLen);
    //printf("write_fifo_data: write %s\n", (ret==0)?"OK":"NG");
...
unsigned char  fm11_read_fifo(unsigned char NumByteToRead, unsigned char *pbuf)
{
    ...
    if (fm11_read_eep(pbuf, FM327_FIFO, read_fifo_len) == 0) {
        //printf("fm11_read_fifo : read OK, read_fifo_len[%d]\n", read_fifo_len);
        return 0;
    } else {
        //printf("fm11_read_fifo : read NG\n");
        return -1;
    } 
}
/*写FIFO*/
