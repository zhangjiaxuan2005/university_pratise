...
#define CONFIG_APP_DEVICEID  "6572de03f672eb209deeb05f_znjj" // 替换为注册设备后生成的deviceid
...
void deal_report_msg(void)
{
    ...
    service.service_id = "znjj_suit";   //服务名
...
static void deal_cmd_msg(cmd_t *cmd)
{
    ...
    if (strcmp(cJSON_GetStringValue(obj_cmdname), "Beep") == 0) {
        ...
        obj_para = cJSON_GetObjectItem(obj_paras, "ON_OFF");//参数名称：args
        if (obj_para == NULL) {
            cJSON_Delete(obj_root);
        }
        ///< operate the LED here
        if (strcmp(cJSON_GetStringValue(obj_para), "ON") == 0) {//数据类型：string
            g_app_cb.led = 1;
            IoTPwmStart(0, 50, 4000);
            printf("huawei_cloud_mqtt: get cmd: Beep(ON)\n");
        } else {
            g_app_cb.led = 0;
            IoTPwmStop(0);
            printf("huawei_cloud_mqtt: get cmd: Beep(OFF)\n");
        }
        cmdret = 0;
        oc_cmdresp(cmd, cmdret);
    }
